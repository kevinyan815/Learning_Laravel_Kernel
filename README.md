...
如果你对`laravel`里面的依赖注入、服务绑定、服务解析等等这些东西很好奇，并且觉得只有理解了一个框架的核心代码才能真正把一个框架用好才能写出最佳实践，那么这个教程对你会很有帮助。教程完整覆盖了 laravel 核心的所有内容，并且根据开发者使用 Laravel 的通用场景开始逐步深入内核讲解整个框架核心流程中涉及到的方方面面，整个教程的目录顺序也是根据用Laravel进行开发时常涉及的部分编排的。相信你认真学完这个教程自己融汇贯通后就能完全掌握 Laravel 并胜任用它设计和架构生产系统的职责。
## 面向的人群
